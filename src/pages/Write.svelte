...
function writeUserData(imgUrl) {  
  push(ref(db, "items/"), { //title 값이 중복일때를 막는 push
    title,
    price,
    description,
    place,
  ...
  alert("글쓰기가 완료 되었습니다.");
  window.location.hash ="/"; //다시 홈으로 이동
}
...
const uploadFile = async () => {
  ...
  const url = await getDownloadURL(imgRef); //이미지 업로드 된 실제 주소로 이미지를 크게 볼수있다.
  return url; //받은 url 기반으로
 };

 const handleSubmit = async () =>{
  const url = await uploadFile(); //이미지를 업로드하고
writeUserData(url); //user data를 업로드
 }
</script>
...
<form id="write-form" on:submit|preventDefault={handleSubmit}>
    ...
    <div>
      ...
      <input type="text" id="title" name="title" bind:value={title} /> <!--bind:value 를 타이틀에 적용-->
    </div>
